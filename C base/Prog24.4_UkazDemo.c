/* Эта программа демонстрирует указатели с помощью объявления и инициализации обычных переменных и 
переменных-указателей типа int, float и char, а затем отображает значения каждой из переменных */
...
main() 
{
	...
	price = 17.50;
	pPrice = &price;
	printf("Скольких детей вы ведете в аквапарк? "); 
	scanf(" %d", &kids);
	pKids = &kids;
	printf("\nУ вас есть скидка?");
	printf("Введите Е для скидки сотрудника, S для скидки по программе Sav-More ");
	printf("или N, если нет скидки:");
	scanf(" %c", &code);
	pCode = &code;
	...
	/* Теперь повторим тот же самый код, но использовав для тех же результатов 
	разименованные указатели */
	...
	case ('S'): 
		printf("Скидка Sav-more позволяет сэкономить 15 %% от цены ");
		printf(" $%.2f price", *pPrice);
		printf("\nОбщая стоимость билетов: $%.2f", (*pPprice * .85 * *pKids));
		break;
	default: /* Если введена N при отсутствии скидки или неправильная буква */
		printf("Вы заплатите полную стоимость билетов, ");
